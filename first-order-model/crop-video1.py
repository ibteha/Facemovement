...
def compute_bbox(start, end, fps, tube_bbox, frame_shape, inp, image_shape, increase_area=0.1, count=0):
    ...
    dir_name = args.inp.split('/')[-1].split('.')[0]
    directory = f'./cropped-output/{dir_name}'
    if not os.path.exists(directory):
        os.makedirs(directory)
    stime = str(time.time())
    out = f'./cropped-output/{dir_name}/crop{stime}.mp4'
    return f'ffmpeg -i {inp} -ss {start} -t {time123} -filter:v "crop={w}:{h}:{left}:{top}, scale={scale}" {out}'
...
def compute_bbox_trajectories(trajectories, fps, frame_shape, args):
    ...
    for i, (bbox, tube_bbox, start, end) in enumerate(trajectories):
        if (end - start) > args.min_frames:
            command = compute_bbox(start, end, fps, tube_bbox, frame_shape, inp=args.inp, image_shape=args.image_shape, increase_area=args.increase, count=i)
